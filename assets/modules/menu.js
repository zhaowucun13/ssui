	...
	ss_menu.prototype = {
		...
		init: function() {
			var self = this,
				obj = self.sourceObj;
			//渲染前->进行权限管理
			obj.isJurisdiction && this.lg_dpMuDataFn();
			this.rd_menuWrap();
			this.rd_menuItems();
			this.theEndFn();
			this.lg_f5Open();
			obj.isJurisdiction && this.lg_clearDomFn(); //数据权限后，没项的dom进行清除
					...
					for(var a = 0; a < heelerData.length; a++) {
						if(heelerData[a]) {
							var itemWrap = ss.crtDom('div', 'wrap_' + count, '', appendToWrap, {
								...
								cv: ['100%', heelerData[a].isShow == false ? '0px' : '50px', 'border-box', 'hidden', 'all .3s', heelerData[a].isShow == false ? 'none' : 'block'],
								an: ['count', a === 0 && 'first','isShow'],
								av: [count, a === 0 && 'true',heelerData[a].isShow]
							}, [
								//每一项的点击
								'click',
								function(dom, e) {
									...
									if(dom.children.length > 1) {
										if(dom.getAttribute('isOpen') && dom.getAttribute('isOpen') == 'true') {
											...
											var itemH = 0;
											for(var h = 0; h < dom.children.length; h++) {
												var _forCurDom = dom.children[h];
												var isShow = _forCurDom.getAttribute('isShow')
												var curItem = _forCurDom.nodeName == 'P' ? '50px' : _forCurDom.style.height;
												curItem && isShow != 'false' && (itemH = itemH + Number(curItem.slice(0, curItem.indexOf('px'))))
											}
											var tempDom = dom;
											if(count > 0) {
												for(var a = count; a > 0; a--) {
							...
							//为可以点击项(有txt字段)
								...
								//设置资源码
								heelerData[a].sourceCode && leaderDom.setAttribute('sourcecode', heelerData[a].sourceCode);
								//文字
								ss.crtDom('span', 'txt', heelerData[a].leader, leaderDom, {
				...
				if(location.hash.slice(1) != 'home') {
					...
					var disposeFn = function(curWrap, curCount, flag) {
						...
						for(var h = 0; h < curWrap.children.length; h++) {
							var curItem = curWrap.children[h].tagName == 'P' ? '50px' : curWrap.children[h].style.height;
							var isShow = curWrap.children[h].getAttribute('isShow')
							//var curItem = '50px';
							curItem && isShow != 'false' && (itemH = itemH + Number(curItem.slice(0, curItem.indexOf('px'))))
						}
			...
			var disposeFn = function(shadowCss, boolean, colorVal, degVal, callback) {
				...
				if(boolean == 'true' && count === 0) {
					...
					for(var a = 0; a < tempWrapDom2.length; a++) {
						...
						curDomP_icon2 && (curDomP_icon2.innerHTML = ss.svgRepository.chart('16', '#bdbdbd'));
						curDomP2.style.boxShadow = 'none';
						curDomP_iconSj2 && (curDomP_iconSj2.style.transform = 'rotate(0deg)');
						curDomP_txt2 && (curDomP_txt2.style.color = '#bdbdbd');
					}
				};
		...
		//渲染->进行权限控制(暂无使用)
		lg_sourceBehindFn: function() {
			var self = this,
				obj = self.sourceObj;
			//当前角色的菜单资源数据
			var curRoleSourceData = obj['sourceData'];
			for(var u in curRoleSourceData) {
				var curDom = ss.getDom('[sourcecode="' + u + '"]');
				var curWrap0 = self.lg_searchUpFatherDomFn(curDom, {
					count: '0'
				});
				console.log(curWrap0);
			};
		},
		//往上查找父级元素->直至条件当前dom的条件满足
		lg_searchUpFatherDomFn: function(sonDom, obj) {
			//跳出递归条件
			var condition = obj;
			//条件对象的key
			var key = '';
			...
			//最后的dom
			var endDom;
			var recursion = function(dom) {
				var curSonParentNode = dom.parentNode;
				if(curSonParentNode.getAttribute(key) == condition[key]) {
					return(endDom = curSonParentNode);
				} else {
					recursion(curSonParentNode);
				};
			};
			recursion(sonDom);
			return endDom;
		...
		//菜单数据->进行权限控制(使用)
		lg_dpMuDataFn: function(data) {
			var self = this,
				obj = self.sourceObj;
			var curDpData = ss.options['menuData'];
			//跳出递归条件
			var condition = function() {

			};
			//判断数组中是否含有sourceCode的项
			var judgeArr = function(arr) {
				var curArr = arr;

			};
			//递归函数
			var recursion = function(dpData) {
				var curLeaderArr = dpData['heeler'];
				for(var c = 0; c < curLeaderArr.length; c++) {
					if(curLeaderArr[c]['sourceCode']) {
						//不存在资源码->删除属性
						obj['sourceData'][curLeaderArr[c]['sourceCode']] || (delete curLeaderArr[c]);
					} else {
						//不存在资源码
						if(curLeaderArr[c]['heeler'].length != 0) {
							//存在下一项的数组
							recursion(curLeaderArr[c]);
						};
					}
				};
			};
		...
		//菜单数据->数据权限->dom控制
		lg_clearDomFn: function() {
			var self = this,
				obj = self.sourceObj;
			var needClearDom = [];
			//第一层dom清除
			var firstWrapDom = self['domWrap']['mn_content'].children;
			for(var f = 0; f < firstWrapDom.length; f++) {
				var isTF = false;
				var curSons = firstWrapDom[f].children;
				for(var s = 0; s < curSons.length; s++) {
					if(curSons[s].nodeName == 'DIV') {
						isTF = true;
						break;
					};
				};
				if(!isTF) {
					needClearDom.push(firstWrapDom[f]);
				}
			};
			for(var l = 0; l < needClearDom.length; l++) {
				needClearDom[l].parentNode.removeChild(needClearDom[l]);
			};
		},
		//<--- | 容器 | --->
		domWrap: {},
