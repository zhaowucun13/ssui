...
;!function(win){
    ...
    ss.prototype = {
        ...
        init : function(){
            ...
            self.dpDocArrFn();//整理文档路径数组
        },
        //<--- | 模块加载器(简易版) | --->
        //定义参数
        config:function(queryObj){
            var self = this,
                obj = self.sourceObj, 
                paths = queryObj.private,
                incomings = queryObj.incoming;
            self.scope.mdWrap = queryObj;
            incomings && self.getObjleg(incomings)!=0 && (
                function(){
                    for(var x in incomings){
                        self.scope.mdPaths[x] = self.getPath(incomings[x]);                        
                    }
                }()
            );
            paths && self.getObjleg(paths)!=0 && (
                function(){
                    for(var x in paths){
                        self.scope.mdPaths[x] = self.getPath(paths[x]);                        
                    }
                }()
            );
            return self;
        ...
        set : function(para,callback){
            ...
            (
                ...
                callback(function(mdName,mdCode){
                    self[mdName] = mdCode;
                    self.scope.mdStatus[mdName] = true;
                })
            )
                :
            self.use(typeof para==='string' ? [para] : para,callback,'set')
        },
        //使用模块
        use : function(para,callback,type){
            var self = this,obj = self.sourceObj,para = para;
            ...
            function isPrivate (item){
                return self.scope.privateMd.indexOf(item)>-1 
                    ?
                self.getPath(item)+'.js'
                    :
                false
            }
            //装载依赖的模块
            para.forEach(function(item){
                self.scope.mdStatus[item] || self.loadMd({
                    url: isPrivate(item) ? isPrivate(item) : self.scope.mdPaths[item],
                    mdname:item,
                    cbFn:function(mdname){
                        //判断是否为三方库，重置其状态
                        self.scope.mdWrap.incoming && self.scope.mdWrap.incoming[mdname]
                            && 
                        (self.scope.mdStatus[mdname] = true);
                    }
                });
            ...
            self.polling({
                condition:function(){
                    var isTF = true;
                    para.forEach(function(item){
                        self.scope.mdStatus[item] || (isTF=false);
                    });
                    return isTF;
                },
                cbFn:function(){
        ...
        //装载模块
        loadMd:function(queryObj){
            var self = this,obj = self.sourceObj,headDom = self.getDom('head');
            //创建script标签
            var node = self.crtDom('script','','',headDom,{
                an:['async','charset','src','mdname'],
                av:[true,'utf-8',queryObj.url,queryObj.mdname]
            });
            //监听load事件
            node.addEventListener('load', function(e){
                if (e.type === 'load' || (/^(complete|loaded)$/.test((e.currentTarget || e.srcElement).readyState))) {
                    queryObj.cbFn && queryObj.cbFn(node.getAttribute('mdname'));//存在->则回调
                    headDom.removeChild(node);//移除script节点
                }
            }, false);            
        },
        //获取ssui.js的路径
        ...
        getPath:function(str){
            var self = this,obj = self.sourceObj,
                zeroIndexSrc = self.scope.docSrc['zeroIndexSrc'],
                curIndexArr = self.scope.docSrc['curIndexArr'];
            //获取ssui.js当前的src路径
            if(/\.\./.test(str)){
                var _arr = str.split('../'),
                    _index = _arr.length-1,//具备的层级
                    _curIndexArr = curIndexArr.slice(0);
                if(_index>_curIndexArr.length-1){
                    self.error('当前路径：'+str+'有误');
                    return;
                }
                else{
                    var jsFileNameExc = /(\.\.\/)+(.+)/.exec(str),
                    _jsFileName = jsFileNameExc[jsFileNameExc.length-1],
                    tempSrc = '';
                    _curIndexArr.splice(0,_curIndexArr.length-1-_index).forEach(function(item){
                        tempSrc = tempSrc+'/'+item;
                    });
                    return zeroIndexSrc+tempSrc+'/'+_jsFileName;
                };
            }
                ...
                _arr.forEach(function(item){
                    item && item!='.' && (
                        _jsFileName = _jsFileName+'/'+item
                    );
                });
                var tempSrc = '';
                _curIndexArr.splice(0,_curIndexArr.length-1).forEach(function(item){
                    tempSrc = tempSrc+'/'+item;
                });
                return zeroIndexSrc+tempSrc+_jsFileName;
            }
                ...
                //同级
                var _arr = str.split('/'),
                    _jsFileName = _arr[_arr.length-1],//获取js名，支持三种情况./test.js /test.js test.js 
                    tempSrc = '';
                var _curIndexArr = curIndexArr.slice(0);//拷贝数组
                _curIndexArr.splice(0,_curIndexArr.length-1).forEach(function(item){
                    tempSrc = tempSrc+'/'+item;
                });
                return zeroIndexSrc+tempSrc+'/'+_jsFileName;
        ...
        dpDocArrFn:function(){
            var self = this,obj = self.sourceObj;
            self.scope['docSrc'] = {};//初始化数组
            var curDocSrc = self.getCurDocSrc();//当前ssui的文档地址
            var sliceSrcArr = curDocSrc.slice(curDocSrc.indexOf('://')+3).split('/');//文档截取的数组
            //0层次的路径
            self.scope.docSrc['zeroIndexSrc'] = curDocSrc.slice( 0,curDocSrc.indexOf(sliceSrcArr[0])+sliceSrcArr[0].length );
            //当前具备的层级数组
            self.scope.docSrc['curIndexArr'] = sliceSrcArr.splice(1);
        },
        //轮询器
        polling:function(queryObj){
            var self = this,obj = self.sourceObj,queryObj = queryObj;
            var timeVal = queryObj.time || 4;
            var endTimeout = queryObj.timeout || 5000;
            var _timer;		
            //存在停止条件
            queryObj.condition && (
                _timer = win.setInterval(function(){
                    timeVal = timeVal+timeVal;
                    //设置超过多少毫米自主取消
                    // if(queryObj.condition() || timeVal>endTimeout){
                    if(queryObj.condition()){
                        window.clearInterval(_timer);
                        queryObj.cbFn && queryObj.cbFn();	
                    }
                },queryObj.time||4)
            )
        },
        ...
        load:function(para){
            ...
            var para = typeof para === 'string' ? [para] : para;
            //装载依赖的模块
            para.forEach(function(item){
                self.scope.mdStatus[item] || self.loadMd({
                    url:self.scope.mdPaths[item],
                    mdname:item,
                    cbFn:function(mdname){
                        //判断是否为三方库，重置其状态
                        self.scope.mdWrap.incoming && self.scope.mdWrap.incoming[mdname]
            ...
            executeFn:function(self){
                ...
                try{
                    self.getDom('body').onclick = function(e){
                    	(function(){
                    		var svgH = 20;
	                  		var wrapDom = document.getElementById('_roleWrap');
	                  		if(wrapDom){
		                  		var svgDom = wrapDom.parentNode.querySelector('.svg');
	                    		wrapDom.style.display = 'none';
	                    		wrapDom.parentNode.setAttribute('ischeck','false');
	                    		svgDom.innerHTML = self.svgRepository.sjDown(svgH,'#666');
	                  		}
