ss.set(function(out){
    ...
    Menu.prototype = {
        ...
        theEndFn: function () {
            var self = this,
                obj = self.sourceObj;
            (obj.el||document.body).appendChild(this.domWrap.mn_Container);//追加到文档
        },
        //<--- | 渲染 | --->
        //渲染导航条的容器
        rd_menuWrap:function(){
            ...
            var mn_Container = ss.crtDom('div','mn_Container','',tempContainer,{
                ...
                .appendDom([
                    ...
                    //系统名字
                    ss.crtDom('div','ly_munuName',(obj.name||'ssui管理系统'),'',{
        ...
        rd_menuItems:function(){
                    ...
                    for(var a=0; a<heelerData.length; a++){
                    	if(heelerData[a]){
	                        var itemWrap = ss.crtDom('div','wrap_'+count,'',appendToWrap,{
	                            cn:['width','height','boxSizing','overflow','transition'],
	                            cv:['100%','50px','border-box','hidden','all .3s'],
	                            an:['count',a===0&&'first'],
	                            av:[count,a===0&&'true']
	                        },[
	                            //每一项的点击
            ...
            recursionCreateFn(obj.sourceData,self.domWrap.mn_content,count);
        },
        //<--- | 逻辑 | --->
        //刷新->展开url的hash值各项
        lg_f5Open : function(){
        ...
        lg_sourceBehindFn:function(){
        	...
        	for(var u in curRoleSourceData){
        		...
        		var curWrap0 = self.lg_searchUpFatherDomFn(curDom,{count:'0'});
